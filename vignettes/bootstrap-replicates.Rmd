...
library(labelled) # For variable/value labels
...
-   `make_gen_bootstrap_weights()`, which creates a matrix of replicate weights using the generalized survey bootstrap method. This function will usually be used in conjunction with the function `make_quad_form_matrix()`, which returns the quadratic form matrix representation of common variance estimators.
...
Essentially every bootstrap method commonly used for surveys can be used for simple random sampling with replacement and can easily be applied to stratified sampling (simply repeat the method separately for each stratum). However, things become more complicated for other types of sampling and care is needed to use a bootstrap method appropriate to the survey design. For many common designs used in practice, it is possible (and easy!) to use one of the bootstrap methods described in the section of this vignette titled "Basic Bootstrap Methods." If the design isn't appropriate for one of those basic bootstrap methods, then it may be possible to use the generalized survey bootstrap described in a later section of this vignette, which covers a wide range of designs. The interested reader is encouraged to read @mashreghi2016 for an overview of bootstrap methods developed for survey samples.
...
To implement these basic bootstrap methods, there are two options:

##### Option 1. Convert an existing design to a bootstrap design

**Create a survey design object with the `svydesign()` function from the survey package, and then convert this object to a bootstrap replicate design using `as_bootstrap_design()`.** This method works well for designs that can be specified using the `syvdesign()` function, which includes stratified, multistage samples with simple random sampling or PPS sampling (with or without replacement) at each stage.^[Note that a multistage design with PPS at any of the stages must be specified using the arguments `fpc` to specify stage-specific sampling probabilities and must use the argument `pps = "brewer"`. For example, a two-stage design with PPS at one or both stages could be specified using the following: `syvdesign(ids = ~ first_stage_cluster + second_stage_cluster, fpc = ~ first_stage_prob + second_stage_prob, pps = "brewer")`. For a stage with simple random sampling, the probabilities at that stage are just the strata sample sizes divided by the population sizes].
...
##### Option 2. Create replicate factors using special-purpose functions

**Use the function `make_rwyb_bootstrap_weights()` to create a matrix of replicate weights or replicate factors, given information about the survey design.** This approach allows the creation of bootstrap weights for more complicated designs than those that can be represented using the `svydesign()` function. This approach is useful when a multistage sample has a stage that uses Poisson sampling (which is sometimes used in establishment surveys but in other contexts is sometimes used as a model for nonresponse).

In the example, below the function is used to create bootstrap adjustment factors for a survey where the first stage sample was selected using unequal probability sampling without replacement (PPSWOR) and the second stage sample was selected using simple random sampling without replacement (SRSWOR)
...
adjustment_factors <- make_rwyb_bootstrap_weights(
  num_replicates = 500,
  # Sampling unit IDs at each stage
  samp_unit_ids = library_multistage_sample |>
    select(PSU_ID, SSU_ID),
  # Strata IDs at each stage
  # (should be a constant if there was no stratification)
  strata_ids = library_multistage_sample |>
    transmute(PSU_STRATUM = 1, SSU_STRATUM = 1),
  # Selection probabilities of each stage
  samp_unit_sel_probs = library_multistage_sample |>
    select(PSU_SAMPLING_PROB, SSU_SAMPLING_PROB),
  # Sampling method at each stage
  samp_method_by_stage = c("PPSWOR", "SRSWOR"),
  output = "factors"
)
```

Once the adjustment factors have been created, they can be used to create a replicate design object with the function `svrepdesign()`.

```{r}
bootstrap_rep_design <- survey::svrepdesign(
...
Due to the fact that PSUs were selected with probability proportional to the variable `TOTCIR`, estimates for `TOTCIR` and closely-related variables have lower standard errors compared to estimates from a simple random sampling design, despite the use of cluster sampling.

```{r}
bootstrap_rep_design |>
  svymean(x = ~ TOTCIR + TOTINCM, na.rm = TRUE, deff = TRUE)
```

### Generalized Survey Bootstrap

For sample designs with additional complex features beyond the three highlighted above, the generalized survey bootstrap method can be used. This is especially useful for systematic samples and single-PSU-per-stratum designs.
...
-   For single-stage, stratified cluster samples:

    -   The usual variance estimator: $\sum_{h=1}^{H} \left[(1-f_h)\times n_h \times \frac{1}{n _h-1} \sum_{i=1}^{n_h}(y_{hi.} - \bar{y}_{h..})^2 \right]$
...
When sharing a dataset that uses rescaled weights from a generalized bootstrap survey, the documentation for the dataset should instruct the user to use replication scale factor $\frac{\tau^2}{B}$ rather than $\frac{1}{B}$ when estimating sampling variances.
...
While rule-of-thumb values for the number of replicates that should be used (some say 500, others say 1,000), it is advisable to instead use a principled strategy for choosing the number of replicates. One general strategy proposed by @beaumont2012 is as follows:
