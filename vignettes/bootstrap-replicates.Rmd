...
-   `as_gen_bootstrap_design()`, which creates a replicate survey design using the generalized survey bootstrap method as described by @beaumont2012.
...
**Create a survey design object with the `svydesign()` function from the survey package, and then convert this object to a bootstrap replicate design using `as_bootstrap_design()`.** This method works well for designs that can be specified using the `syvdesign()` function, which include: (1) stratified, multistage samples with simple random sampling (with or without replacement) at each stage, and (2) single-stage PPS samples.
...
##### Option 2. Convert an existing design to a bootstrap design

**Use the function `make_rwyb_bootstrap_weights()` to create a matrix of replicate weights or replicate factors, given information about the survey design.** This approach allows the creation of bootstrap weights for more complicated designs than those that can be represented using the `svydesign()` function. For example, when a multistage sample has one or more stage of unequal probability sampling without replacement, or when any of the sampling stages are based on Poisson sampling.
...
For sample designs with additional complex features beyond the three highlighted above, the generalized survey bootstrap method can be used.
